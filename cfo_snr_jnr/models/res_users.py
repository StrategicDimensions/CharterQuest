...
class ResUsers(models.Model):
    ...
    def _create_member(self, values):
        ...
        if self.partner_id:
            values.update({'aspirants_name': self.partner_id.id})
        if values.get('cfo_comp'):
            configuration = self.env['cfo.competition'].browse(values.get('cfo_comp'))
            if configuration.cfo_comp == 'CFO SNR':
                values.update({'cfo_comp': 'CFO SNR'})
                member = self.env['cfo.snr.member'].create(values)
            elif configuration.cfo_comp == 'CFO JNR':
                values.update({'cfo_comp': 'CFO JNR'})
                member = self.env['cfo.jnr.member'].create(values)
